...
#             logger.info(f"Track count: {len(gpx.tracks)}")
#             logger.info(f"Segment count: {len(gpx.tracks[0].segments[0].points)}")
#             reduced_gpx = reduce_gpx_data_based_on_length(gpx)
#         except Exception as e:
#             logger.error(f"Parsing error: {e}")
#             return

#         elevation_data = [p.elevation for p in reduced_gpx.tracks[0].segments[0].points]
#         lat_data = [p.latitude for p in reduced_gpx.tracks[0].segments[0].points]
#         lon_data = [p.longitude for p in reduced_gpx.tracks[0].segments[0].points]

#         moving_data = [0]
#         for i in range(1, len(lat_data)):
#             moving_data.append(
#                 moving_data[i - 1] + distance(lat_data[i - 1], lon_data[i - 1], lat_data[i], lon_data[i])
#             )
...
#         else:
#             num_ticks = min(max_ticks, max(2, int(max_distance) + 1))
#             dash_line_moving = np.linspace(0, max_distance, num_ticks)
...
#         # Set x-axis to show full numbers only without commas
#         ax.xaxis.set_major_locator(MaxNLocator(integer=True))

#         # Customize y-axis label format with "m"
#         ax.yaxis.set_major_formatter(FuncFormatter(meter_formatter))

#         # Set y-axis tick label color to 80% black and font to Helvetica Neue LT Pro
#         for label in ax.get_yticklabels():
#             label.set_color((0, 0, 0, 0.8))  # RGBA with alpha=0.8 for 80% black
#             label.set_fontname("Helvetica Neue LT Pro")
...
#         f = interp1d(new_moving_data_array, elevation_data_smooth, kind='slinear')
